...
void ambulance_a() {
  ...
  HAL_GPIO_TogglePin(led[3].port, led[3].pin);
  HAL_GPIO_TogglePin(led[5].port, led[5].pin);
  HAL_GPIO_TogglePin(led[7].port, led[7].pin);
  HAL_Delay(500);
  if (ambulance_mode == MODE_B || ambulance_mode == MODE_OFF) {
    all_off();
    return;
  }
  if (ambulance_mode == MODE_C || ambulance_mode == MODE_OFF) {
    all_off();
    return;
  }
  if (ambulance_mode == MODE_OFF || ambulance_mode == MODE_OFF) {
    all_off();
    return;
  }
}
