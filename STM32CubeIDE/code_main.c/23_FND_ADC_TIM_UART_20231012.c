...
int adc_value;
volatile int ADC_1, ADC_2, ADC_3, ADC_4;
/* USER CODE END PV */
...
void display(uint8_t num, uint8_t position, uint32_t time){
   int i;
   for(i = 0; i < 4; i++){
      if(i == position){
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
     display(number[ADC_1], 3, 5);
     display(number[ADC_2], 2, 5);
     display(number[ADC_3], 1, 5);
     display(number[ADC_4], 0, 5);
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
}
