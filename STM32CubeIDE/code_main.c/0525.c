...
typedef struct led {
 ...
 void led_all_off(){
    for(uint8_t k=0; k<8; k++){
       HAL_GPIO_WritePin(led[k].port, led[k].pin, 0);
    }
 }
...
void ambulance_a() {
   for (uint8_t i = 0; i < 4; i++) {
      ...
      HAL_GPIO_WritePin(led[i + 4].port, led[i + 4].pin, 0);
   }
   HAL_Delay(300);
   ...
   for (uint8_t i = 0; i < 4; i++) {
      ...
      HAL_GPIO_WritePin(led[i + 4].port, led[i + 4].pin, 1);
   }
   HAL_Delay(300);
   if(ambulance_mode == MODE_B) {
      led_all_off();
...
void ambulance_b() {
   ...
   for (uint8_t j = 6; j > 0; j--) {
      HAL_GPIO_WritePin(led[j].port, led[j].pin, 1);
      HAL_Delay(100);
      HAL_GPIO_WritePin(led[j].port, led[j].pin, 0);
      HAL_Delay(100);
      if(ambulance_mode == MODE_A) {
         led_all_off();
         return;
...
int main(void) {
   ...
   while (1) {
      if(ambulance_mode == MODE_A) ambulance_a();
      else ambulance_b();
      /* USER CODE END WHILE */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
   if (!HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_2)) {
      if (led_state == LED_ON) {
         HAL_GPIO_WritePin(GPIOB, GPIO_PIN_15, 0); //control+space
